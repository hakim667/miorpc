...
const EditUser = ({ handleClose, user, availableUsernames }) => {
  ...
  const handleEdit = async () => {
    ...
    if (availableUsernames.includes(tempNom + "." + tempPrenom)) {
      alert("Un utilisateur avec le meme username existe déja");
      return;
    }
    setLoading(true);
    console.log(
  ...
  return (
    <>
      <FontAwesomeIcon
        ...
        onClick={handleClose}
      />
      <h1 className=" w-full text-2xl font-bold text-center">Edit User</h1>
      <div className=" w-[90%] flex flex-col gap-4">
        ...
        >
          <option value="none">Agent ou signataire</option>
          <option value="agent">Agent</option>
          <option value="signatorie">Signataire</option>
        </select>
        <select
          defaultValue={laStructure}
          className=" w-full h-10 rounded-2xl text-xl pl-2"
          onChange={(e) => setLaStructure(e.target.value)}
        >
